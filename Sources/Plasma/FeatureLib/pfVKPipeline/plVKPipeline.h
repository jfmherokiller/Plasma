...
*==LICENSE==*/
#ifndef _plVKPipeline_inc_
#define _plVKPipeline_inc_
...
public:
    plVKEnumerate() {
        hsG3DDeviceSelector::AddDeviceEnumerator(&plVKEnumerate::Enumerate);
    }
...
class plVKPipeline : public pl3DPipeline<plVKDevice>
{
    friend class plGLPlateManager;

public:
    plVKPipeline(hsWindowHndl display, hsWindowHndl window, const hsG3DDeviceModeRecord *devMode);
    virtual ~plVKPipeline() = default;

    CLASSNAME_REGISTER(plVKPipeline);
    GETINTERFACE_ANY(plVKPipeline, plPipeline);
...
private:
    static plVKEnumerate enumerator;
};

#endif // _plVKPipeline_inc_
